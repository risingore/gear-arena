...
/**
 * Credits-scroll speed in px/s. Tuned so the 百鬼夜行 reveal lands as the
 * 158.76s bgm_victory finishes. Was 18 (target 113s @ ~2030px content),
 * but the rendered content runs taller (panel padding, line-height,
 * 百鬼夜行 block), so music finished before the scroll did. Adjust in
 * 2-4 px increments — higher = faster scroll, RETURN appears earlier
 * relative to music.
 */
const DEFAULT_SCROLL_SPEED_PX_PER_SEC = 23;
...
export function mountEndingScrollOverlay(opts: EndingScrollOverlayOptions): () => void {
  ...
  const baseSpeed = opts.scrollSpeed ?? DEFAULT_SCROLL_SPEED_PX_PER_SEC;
  let scrollOffset = 0;
  let raf = 0;
