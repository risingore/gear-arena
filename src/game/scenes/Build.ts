...
import { playMusic, queueMusic, MUSIC_KEYS } from '../systems/music';
...
export class Build extends Scene {
  ...
  preload(): void {
    // Pre-fetch this screen's BGM *and* the upcoming battle's combat track
    // (regular vs boss). The loadout screen is dwell time the player spends
    // arranging parts, so the Battle scene that follows never blocks on a
    // download — already-cached tracks (most of a run) are skipped.
    const state = getRunState(this);
    const round = state.generatedRounds[state.currentRound - 1];
    queueMusic(this, MUSIC_KEYS.build, round?.isBoss ? MUSIC_KEYS.boss : MUSIC_KEYS.battle);
  }
