 ...
 * pre-count their first SOUL STRIKEs. Weighted: 1 (20%) / 2 (40%) / 3 (40%)
 * — expected 2.2. Other enemies keep the static value from enemies.ts.
 */
const rollEnemyShieldCharges = (def: EnemyDef | undefined): number => {
  if (def?.id === 'boss_yuki_onna') {
    const r = Math.random();
    return r < 0.2 ? 1 : r < 0.6 ? 2 : 3;
  }
  return def?.shieldCharges ?? 0;
};
...
export class Battle extends Scene {
  ...
  create(): void {
    ...
    this.enemy = {
      ...
      shieldCharges: oneShot ? 0 : rollEnemyShieldCharges(originalDef),
    ...
    // Screenshot Mode C forces every halo tier on so the embed-bg shot
    // gets the full white + gold + orange storm regardless of equipped
    // synergies. The override is dev-only (URL-driven) so prod halo
    // gating is unaffected.
    const screenshotModeForHalo = getScreenshotMode();
    const effectiveSynergyCount = screenshotModeForHalo === 'C' ? 8 : synergyCount;
    if (effectiveSynergyCount >= 3) {
      const HALO_DEPTH = -1;
    ...
    // Debug elapsed-time counter removed by design decision 2026-04-26.
    ...
    // === Screenshot mode for itch.io embed background ===
    // Seeds the YUKIME-Ω face-off (via Preloader), forces the synergy
    // halo to its top tier (handled in the synergy block above), keeps
    // the full HUD, and freezes the scene 2s after create() so the
    // capture catches a motionless climactic frame.
    if (getScreenshotMode()) {
      // Freeze via window.setTimeout (not this.time.delayedCall) so the
      // call fires even after we pause the scene clock. Intro tween
      // chain ≈ 1.1s + fadeInCurrent ≈ 0.3s, so 2s lands well after.
      window.setTimeout(() => {
