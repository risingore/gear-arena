...
export class Select extends Scene {
  ...
  preload(): void {
    // Pre-fetch the Build BGM during character selection so the loadout
    // screen (and the run's first combat track it then pulls) aren't all
    // downloaded in one go on Build's first entry.
    queueMusic(this, MUSIC_KEYS.build);
  }
